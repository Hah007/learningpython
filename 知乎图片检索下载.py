...
class zhihu(object):
    def __init__(self):
        ...
        self.var = StringVar()
        self.label_tips = tkinter.Label(
            self.root, textvariable=self.var, justify=CENTER, font=("微软雅黑", 12, "bold"), fg='red')
        # self.var.set('正在下载，请稍等... (共433个回答)')

        self.ok_var = StringVar()
        self.label_ok = tkinter.Label(self.root, textvariable=self.ok_var, justify=CENTER, font=(
            "微软雅黑", 10, "bold"), fg='Firebrick')
        # self.ok_var.set('已完成 13%')

        self.input_var = StringVar()
        self.label_wtid = tkinter.Label(self.root, text='请输入问题ID或问题链接：')
        self.input_wtid = tkinter.Entry(
            self.root, textvariable=self.input_var, width=37)
        self.button_wtzt = tkinter.Button(
            self.root, text="粘贴", width=6, command=self.getCopyTxet)

        self.wt_file_var = StringVar()
        self.wt_file_var.set('D:/ZhiHu')
        self.label_wt_file = tkinter.Label(self.root, text='请选择保存目录：')
        self.input_wt_file = tkinter.Entry(
            self.root, textvariable=self.wt_file_var, state=DISABLED, width=37)
        self.button_wt_file = tkinter.Button(
            self.root, text="更改", width=6, command=self.thread_browse_folder)

        self.button_wtjc = tkinter.Button(
            self.root, text="检测", width=8, command=self.thread_wenti_jc, font=("微软雅黑", 12, "bold"))
        ...
        self.label_tips3 = tkinter.Label(
            self.root, text='2.问题资源为实时下载，你可随时在下载文件夹查看')
    ...
    def wenti_jc(self):
        ...
        if len(wt_data) == 0:
            ...
            try:
                if wt_data.isdigit():
                    url = 'https://www.zhihu.com/api/v4/questions/{}/answers'.format(
                        wt_data)
                    ...
                    if r.status_code == 200:
                        ...
                        tkinter.messagebox.showinfo(
                            '问题ID正确', '你本次要下载的问题为“%s”' % self.title)
                    else:
                        tkinter.messagebox.showerror(
                            '问题ID输入错误', '请检查你的问题ID并重新输入')
                else:
                    ...
                    if wtids and len(wtids[0]) != 0:
                        ...
                        if r.status_code == 200:
                            ...
                            self.id = int(wtids[0])
                            tkinter.messagebox.showinfo(
                                '问题链接正确', '你本次要下载的问题为“%s”' % self.title)
                        else:
                            tkinter.messagebox.showerror(
                                '问题链接输入错误', '请检查你的问题链接并重新输入')
                    else:
    ...
    def get_urls(self, offset):
        try:
            ...
            for data in datas:
                ...
                video_names = []
                img_urls = re.findall(
                    '<noscript><img src="(.*?)"', content, re.S)
                video_urls = re.findall(
                    '"z-ico-video"></span>(.*?)</span>', content, re.S)
                if img_urls:
                    for i in range(len(img_urls)):
                        file_name = '{}({})_{}'.format(
                            name, otherStyleTime, i + 1)
                ...
                if video_urls:
                    ...
                    if len(video_ids) == len(video_names):
                        ...
                        pool = threadpool.ThreadPool(20)
                        results = threadpool.makeRequests(
    ...
    def save_video(self, video_id, video_name):
        try:
            ...
            video_url = requests.get(url, headers=self.headers).json()[
                'playlist']['LD']['play_url']
